...
void interruptionListener(	void *inClientData, UInt32 inInterruptionState) {
    ...
    if (inInterruptionState == kAudioSessionBeginInterruption) {
		...
		//if (result) NSLog("Error setting audio session active! %d", result);
...
- (void) vibrate
{
	//if(![[SettingManager sharedSettingManager] vibrateOn])
	//	return;
...
{	
	...
	alSource3f(sourceID, AL_POSITION, listenerPosition.x, listenerPosition.y, 0.0f);
