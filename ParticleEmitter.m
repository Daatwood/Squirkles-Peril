						  ...
						  blendAdditive:(BOOL)inBlendAdditive
{
	...
	if (self != nil) {
		...
		if(inTextureName) {		
			...
			texture = [[Image alloc] initWithImageNamed:inTextureName scale:1.0f filter:GL_NEAREST];
		}
                               ...
                               blendAdditive:(BOOL)inBlendAdditive
{
	...
	if (self != nil) {
		...
		if(inTextureName) {		
			// Create a new texture which is going to be used as the texture for the point sprites
            texture = [[ResourceManager sharedResourceManager] getImageWithImageNamed:inTextureName withinAtlasNamed:inAtlasName];
			//texture = [[Image alloc] initWithImageNamed:inTextureName scale:1.0f filter:GL_NEAREST];
		}
...
- (void)update:(GLfloat)delta 
{
		...
		while(particleIndex < particleCount) 
		{
			...
			if(currentParticle->timeToLive > 0)
			{
				...
				//Vector2f deltaGravity = Vector2fMultiply(Vector2fNormalize(gravity), delta); 
				//Vector2f deltaDirection = Vector2fMultiply(Vector2fNormalize(currentParticle->direction), delta); 										 
				
				currentParticle->direction = Vector2fAdd(currentParticle->direction, gravity);
				//currentParticle->direction = Vector2fAdd(currentParticle->direction, deltaGravity);
				//currentParticle->direction = Vector2fMultiply(currentParticle->direction, delta);
				
				currentParticle->position = Vector2fAdd(currentParticle->position, Vector2fMultiply(currentParticle->direction, delta));
...
- (void)renderParticles 
{
	...
	if(texture && [[Director sharedDirector] currentlyBoundTexture] != [[texture texture] name]) {
		glBindTexture(GL_TEXTURE_2D, [[texture texture] name]);
		[[Director sharedDirector] setCurrentlyBoundTexture:[[texture texture] name]];
	}
