...
SYNTHESIZE_SINGLETON_FOR_CLASS(MultiplayerManager);
...
{
    NSLog(@"Connect to peer: %@", [self displayNameForPeer: peerID]);
...
{
    NSLog(@"Did recieve connection request from peer: %@", [self displayNameForPeer: peerID]);
    if (sessionState == ConnectionStateDisconnected) {
        currentConfPeerID = [peerID retain];
        sessionState = ConnectionStateConnecting;
...
- (void)session:(GKSession *)session peer:(NSString *)peerID didChangeState:(GKPeerConnectionState)state
{
    ...
    { 
		case GKPeerStateAvailable:
            NSLog(@"GK Peer Available: %@", [self displayNameForPeer: peerID]);
            [self connect:peerID];
            /*
		...
		case GKPeerStateDisconnected:
            NSLog(@"GK Peer Disconnected: %@", [self displayNameForPeer: peerID]);
        ...
        case GKPeerStateConnecting:
            NSLog(@"GK Peer Connecting: %@", [self displayNameForPeer: peerID]);
...
@implementation MultiplayerManager (VoiceManager)
